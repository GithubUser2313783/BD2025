...
SELECT * 
FROM [dbo].[tblProduits]
WHERE [ID fournisseur] = 5 AND [ID type de produit] = '8'
...
SELECT
	 ...
	 Titre,
	 Sexe,
	 [Ventes de l'année dernière] VentesAnneesDernieres ,
...
FROM (
		SELECT
			 ...
			 [Titre],
			 CASE WHEN
					[Titre] IN ('mlle', 'mme', 'mrs') THEN 'F' ELSE 'M'
			 END Sexe,
...
-- ATTENTION
-- Une insctruction CREATE/DROP VIEW doit se terminer par GO qui veut dire : "EXECUTION" explicite.
...
SELECT CONCAT([Nom], ' ', [Prénom]) FullName,
	   [Poste occupé],
	   [Salaire]
...
CREATE VIEW vCreditClients AS
SELECT [Numéro d'autorisation de crédit],
	   [ID crédit client],
	   [Montant]
FROM [dbo].[tblCredits];
GO
